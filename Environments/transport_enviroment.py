...
# import environment.twissElement as twiss
...
class transportENV(gym.Env):
    ...
    def _take_action(self, action):
        ...
        if (self.intensity_on_target[0] > 0.8):
            ...
            time_is_over = True
        elif (np.abs(self.beam_pos) > 0.05):
            time_is_over = True
            reward = -reward
        else:
            reward = -reward
    ...
    def reset(self):
        ...
        self.mssb_angle = np.random.uniform(-0.0005, 0.0005, 1)[0]  # rd.uniform(-0.0005, 0.0005)
        self.mbb_angle = np.random.uniform(-0.0005, 0.0005, 1)[0]  # rd.uniform(-0.0005, 0.0005)
